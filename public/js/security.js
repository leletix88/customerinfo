			...
			user.$refresh().then(
				function() {
					...
					if (user.$isAuthenticated() && $scope.remember) {
						...
						$cookies.put('CustomerInfo_user', user.user, opt);
						$cookies.put('CustomerInfo_pass', hash, opt);
					}
...
function($scope, $location, $cookies, UserProfile, $http) {
	...
	$cookies.remove('CustomerInfo_user');
	$cookies.remove('CustomerInfo_pass');
...
function($q, User, $cookies, $http) {
	...
	var fetchUserProfile = function() {
		...
		//IF HAS COOKIES SET 
		if ($cookies.get('CustomerInfo_user')) {
			$http.defaults.headers.common['user'] = $cookies.get('CustomerInfo_user');
			$http.defaults.headers.common['pass'] = $cookies.get('CustomerInfo_pass');
		};
